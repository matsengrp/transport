...
def write_full_replicate_dataset(output_filename, reptag, chain="B"):
    repfiles = sorted(glob('{}{}_*_{}.tcrs'.format(CONFIG["IEL_DATA_DIR"], reptag, chain)))
    dfs = []
...
dn_file = os.path.join(CONFIG["TMP_OUTPUT"], dn_filename)
cd4_file = os.path.join(CONFIG["TMP_OUTPUT"], cd4_filename)
write_full_replicate_dataset(dn_file, "DN")
write_full_replicate_dataset(cd4_file, "CD4")

multi_clusterer = TCRMultiClusterer(file_1=cd4_file, file_2=dn_file, species="mouse", outdir=os.path.join(CONFIG["HMM_OUTPUT"], "cd4_dn"))

pd.DataFrame(multi_clusterer.result).transpose().to_csv(os.path.join(CONFIG["TMP_OUTPUT"], 'multiple_clusters.csv'))
