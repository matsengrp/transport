## this script wants to be run from the transport/ directory
## it can be modified to perform a paired-chain TCRdist analysis, but
## since ALICE only works on beta chains that part is commented out.
##
...
results_file = 'results/flu_spike_ins/flu_auc_values_single_chain.tsv'

lambd = .01 # sinkhorn regularization
random_state = 10
nbrcutoff_single = 48.5 # TCRdist threshold for defining TCR neighbors
Dmax_single = 200 # divide raw TCRdist values by this prior to sinkhorn calc

# we could do a paired analysis by using these values instead, and
# swapping get_paired_distance_matrix in place of get_single_chain_distance_matrix
#
#nbrcutoff_paired = 120.5
#Dmax_paired = 400 # divide raw TCRdist values by this prior to sinkhorn calc

num_naive_list = [1000]
num_flu_list = [5, 10, 20, 40, 80, 160, 320]
num_repeats = 10

tmpfile_prefix = 'tmpfile_flu_spike_ins_single'
...
# not currently used but could swap in for get_single_chain_distance_matrix
...
def get_single_chain_distance_matrix(df1, df2, chain):
    assert chain in ['A','B']
    f1 = get_tmpfile()
    f2 = get_tmpfile()
    matrices = []
    ...
    df1[cols].to_csv(f1, header=False, index=False)
    df2[cols].to_csv(f2, header=False, index=False)
    return get_raw_distance_matrix(f1,f2)
    ...
    for num_flu in num_flu_list:
        for r in range(num_repeats):
            ...
            for chain in 'AB':
                ...
                D_fg_bg = get_single_chain_distance_matrix(fg_df, bg_df, chain)
                D_fg_fg = get_single_chain_distance_matrix(fg_df, fg_df, chain)
                ...
                # total effort matrix
                hadamard = Dmax_single * D * mat

                print(f'sinkhorn_dist: {hadamard.sum():9.3f} '
                      f'lambda: {lambd} Dmax: {Dmax_single} '
                      f' Ns: {N1} {N2}', flush=True)

                ## take row and column (unused) sums of the effort matrix
                ## this is IndividualLoneliness in the paper
                ...
                # save partial results for curiosity
