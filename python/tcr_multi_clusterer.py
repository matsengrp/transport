...
class TCRMultiClusterer():
    def __init__(self, file_1, file_2, species, outdir, max_cluster_count=10):
        ...
        initial_clusterer = TCRClusterer(self_distance_matrix=initial_rep_2_self_dist_mat, score_dict=initial_score_dict, species=self.species, cluster_label="cluster_1")
        ...
        hmmer_manager = HMMerManager(species=self.species)
        if not os.path.exists(outdir):
            os.makedirs(outdir)
        
        hmmer_manager.build_hmm_from_sequences(
            [s.split(',')[1] for s in initial_clusterer.cluster_dict['tcrs']],
            hmm_filename=os.path.join(outdir, 'cluster_1.hmm'),
            alignment_outfilename=os.path.join(outdir, 'cluster_1.sto'),
            fasta_filename=os.path.join(outdir, 'cluster_1.fasta'),
            plot_filename=os.path.join(outdir, 'cluster_1.png')
        )
        cluster = 2
        ...
        while cluster <= max_cluster_count:
            ...
            current_scorer, current_clusterer = self.run_clustering_step(file_1=file_1, file_2=sub_file, cluster_label=cluster, score_dict=score_dict) 
            ...
            hmmer_manager.build_hmm_from_sequences(
                ...
                hmm_filename=os.path.join(outdir, 'cluster_{}.hmm'.format(cluster)),
                alignment_outfilename=os.path.join(outdir, 'cluster_{}.sto'.format(cluster)),
                fasta_filename=os.path.join(outdir, 'cluster_{}.fasta'.format(cluster)),
                plot_filename=os.path.join(outdir, 'cluster_{}.png'.format(cluster))
            )
            cluster = cluster + 1

        self.result = result

    def run_clustering_step(self, file_1, file_2, cluster_label, score_dict=None):
        ...
        tcr_clusterer = TCRClusterer(self_distance_matrix=rep_2_self_dist_mat, score_dict=score_dict, species=self.species, cluster_label=f"cluster_{cluster_label}")
